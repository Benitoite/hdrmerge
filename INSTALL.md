...
```
3. alglib is not available on brew (for now) so [Download ALGLIB 3.16.0 for C++](http://www.alglib.net/download.php) and extract to ~/alglib manually or:
``` bash
mkdir ~/alglib && cd ~/alglib
curl https://www.alglib.net/translator/re/alglib-3.16.0.cpp.gpl.zip --output ~/alglib/ALGLIB.zip
